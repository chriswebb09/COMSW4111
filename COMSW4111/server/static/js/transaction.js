...
function TransactionPage() {
  const [transPostData, setTransPostData] = useState({})
  ...
  const calculateServiceFee = (price) => {
    return (parseFloat(price) * 0.05).toFixed(2); // 5% service fee
  };
  ...
  const handleInputChange = (e) => {
    ...
    setTransaction(prevState => {
      ...
      // Auto-calculate service fee when price changes
      if (name === 'agreed_price') {
        newState.serv_fee = calculateServiceFee(value);
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setTransPostData({
        ...
        serv_fee: formData.serv_fee,
        t_date: transaction.t
      });
...
ReactDOM.render(<TransactionPage />, domContainer);